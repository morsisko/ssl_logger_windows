...
function initializeGlobals()
{
  ...
  var exps = [
    ["ssleay32.dll", ["SSL_read", "SSL_write", "SSL_get_fd", "SSL_get_session", "SSL_SESSION_get_id"]],
    ["ws2_32.dll", ["getpeername", "getsockname", "ntohs", "ntohl"]]
    ];
  for (var i = 0; i < exps.length; i++)
  {
    ...
    {
      var name = names[j];
	  var match = Module.getExportByName(lib, name);
	  
	  if (!match)
		throw "Could not find " + lib + "!" + name;
	
      addresses[name] = match;
    }
  }
